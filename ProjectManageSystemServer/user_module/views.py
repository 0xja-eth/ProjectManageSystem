...
class UserManager:
    ...
    class Check:
        ...
        def ensureNameFormat(cls, val: str):
            if not val:
                raise ErrorException(ErrorType.NameFormate)
            if not len(val)<64:
                raise ErrorException(ErrorType.NameFormate)
        ...
        def ensureGenderFormat(cls, val: str):
            if not val:
                raise ErrorException(ErrorType.IncorrectGender)
        ...
        def ensureCityFormat(cls, val: str):
            if not val:
                raise ErrorException(ErrorType.IncorrectCity)
            if not len(val)<16:
                raise ErrorException(ErrorType.IncorrectCity)
        ...
        def ensureEduIdFormat(cls, val: str):
            if not val:
                raise ErrorException(ErrorType.IncorrectEdu)
        ...
        def ensureDutyFormat(cls, val: str):
            if not val:
                raise ErrorException(ErrorType.IncorrectDuty)
            if not len(val)<16:
                raise ErrorException(ErrorType.IncorrectDuty)
        ...
        def ensureContactFormat(cls, val: str):
            if not val:
                raise ErrorException(ErrorType.IncorrectConnact)
            if not len(val)<16:
                raise ErrorException(ErrorType.IncorrectConnact)
        ...
        def ensureDescFormat(cls, val: str):
            if not val:
                raise ErrorException(ErrorType.IncorrectDescription)
            if not len(val) < 100:
                raise ErrorException(ErrorType.IncorrectDescription)
